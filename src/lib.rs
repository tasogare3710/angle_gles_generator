use ::{
    ...
    gl_generator::{Api, Fallbacks, Generator, Profile, Registry},
    std::{fs::File, io, path::Path},
};

pub fn build_khrplatform(angle_out_home: &Path, rust_target: RustTarget) -> Builder {
...
pub fn build_eglplatform(angle_out_home: &Path, rust_target: RustTarget) -> Builder {
    ...
    E: AsRef<[&'a str]>,
{
    ...
    Registry::new(Api::Egl, version, Profile::Core, fallbacks, extensions).write_bindings(generator, &mut output)
}
...
{
    let mut output = File::create(output)?;
    Registry::new(Api::Gles2, version, Profile::Core, fallbacks, extensions).write_bindings(generator, &mut output)
}
